...
for subjInd=1:length(subjects)
    ...
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_choseGo_rt_high'}=nanmean(rt(PairType==1&Outcome==1));
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_choseNoGo_rt_high'}=nanmean(rt(PairType==1&Outcome==0));
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_choseGo_rt_low'}=nanmean(rt(PairType==2&Outcome==1));
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_choseNoGo_rt_low'}=nanmean(rt(PairType==2&Outcome==0));
    
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_choseGo_rt'}=nanmean(rt(Outcome==1));
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_choseNoGo_rt'}=nanmean(rt(Outcome==0));
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_rt_low'}=nanmean(rt(PairType==2));
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_rt_high'}=nanmean(rt(PairType==1));
    
    SampleSummeryTable{SampleSummeryTable{:,'sub'}==subjects(subjInd),'probe_rt_all'}=nanmean(rt);

    
    
    
    Keep_ProbeData=array2table([data(:,1) ones(length(data),1)*sample_cntr PairType Outcome rt],'VariableNames',{'sub','sample','PairType','Outcome','rt'});
    
    ProbeData=[ProbeData ; Keep_ProbeData];
