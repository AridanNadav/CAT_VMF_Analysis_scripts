...
for subjInd=1:length(subjects)
    ...
    data=Probe_recode_fractals(subjects(subjInd));
    ...
    if order==1 %define which stimuli were Go items: [HighSanity   High   MidHighSanity   MidLowSanity   Low  LowSanity]
        GoStim=[4 5   7 10 12 13 15 18 20 21    24 25    36 37   39 42 44 45 47 50 52 53   56 57];
    elseif order==2
        GoStim=[3 6   8  9 11 14 16 17 19 22    23 26    35 38   40 41 43 46 48 49 51 54   55 58];
    end
    ...
    HH_vs_LL_Sanity=ismember(Rank_left,[3:6,55:58])&ismember(Rank_right,[3:6,55:58]);
    HH_vs_LM_Sanity=ismember(Rank_left,[3:6,35:38])&ismember(Rank_right,[3:6,35:38]);
    HM_vs_LL_Sanity=ismember(Rank_left,[23:26,55:58])&ismember(Rank_right,[23:26,55:58]);
    HM_vs_LM_Sanity=ismember(Rank_left,[23:26,35:38])&ismember(Rank_right,[23:26,35:38]);
...
% Organize data in a summary table
    ...
    probe_results(subjInd,19)=sum(HH_vs_LL_Sanity==1&Outcome==1)/sum(HH_vs_LL_Sanity==1&Outcome~=999); % High-High vs Low-Low Sanity
    probe_results(subjInd,20)=sum(HH_vs_LM_Sanity==1&Outcome==1)/sum(HH_vs_LM_Sanity==1&Outcome~=999); % High-High vs Middle-Low Sanity
    probe_results(subjInd,21)=sum(HM_vs_LL_Sanity==1&Outcome==1)/sum(HM_vs_LL_Sanity==1&Outcome~=999); % Middle-High vs Low-Low Sanity
    probe_results(subjInd,22)=sum(HM_vs_LM_Sanity==1&Outcome==1)/sum(HM_vs_LM_Sanity==1&Outcome~=999); % Middle-High vs Middle-Low Sanity
...
Text{13}='\nHigh-High vs Low-Low Sanity';
Text{14}='High-High vs Middle-Low Sanity';
Text{15}='Middle-High vs Low-Low Sanity';
Text{16}='Middle-High vs Middle-Low Sanity';
...
probe_recognition_table=[probe_for_recognition(:,[1,7,8]),Recognition_results(:,[3,12,13])];
