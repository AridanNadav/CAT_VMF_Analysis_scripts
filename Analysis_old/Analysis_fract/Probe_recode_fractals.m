...
function [subject_data]=Probe_recode_fractals (subj_id_num)

analysis_path=pwd;
outpath=[analysis_path(1:end-8),'Output/'];

    filename=strcat(outpath,sprintf('BMI_bfr_%d',subj_id_num));
    logs=dir(strcat(filename, '_probe_block','*.txt')) ;
    subject_data=[];
    for datafile = 1:length(logs)
        ...
        fclose(fid);
  subject_data(end+1:end+80,:)=cell2mat(Data);     
    end
