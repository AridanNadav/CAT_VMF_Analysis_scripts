...
subjects= [  148 149 150 151 152 153 154  156 157 158 159 160 161 162 163 177 178 179 180 181 182 183 184] %messed up probe until 148. 155 didn't understand instructions of probe
...
for subjInd=1:length(subjects)
   ...
   filename=strcat(outpath,sprintf('BM2_0%d_BDM1.txt',subjects(subjInd)));
   
   else
   filename=strcat(outpath,sprintf('BM2_%d_BDM1.txt',subjects(subjInd)));
   end
...
sortedM1_by_itemname=sortedM1(:,2)
   ...
   filename=strcat(outpath,sprintf('BM2_0%d_BDM2.txt',subjects(subjInd)));
   
   else
   filename=strcat(outpath,sprintf('BM2_%d_BDM2.txt',subjects(subjInd)));
   end
...
switch order
    ...
    case 1
BDM_1_Ex_O1(O1_count,1)=mean(sortedM1([8 11 12 15   16 19 20 23 ],1)); %stop High ?
BDM_1_Ex_O1(O1_count,2)=mean(sortedM1([9 10 13 14  17 18 21 22  ],1)); % go High
BDM_1_Ex_O1(O1_count,3)=mean(sortedM1([ 38 41 42 45 46 49 50 53] ,1)); % stop Low
BDM_1_Ex_O1(O1_count,4)=mean(sortedM1([  39 40 43 44 47 48 51 52],1)); % go Low
BDM_1_Ex_O1(O1_count,5)=subjects(subjInd); % go Low
BDM_1_Ex_O1(O1_count,6)=order; % go Low

names1{O1_count}=sortedlist1([8 11 12 15    ])

BDM_2_Ex_O1(O1_count,1)=mean(sortedM2(sortedM1([8 11 12 15   16 19 20 23],2),2)); %stop High ?
BDM_2_Ex_O1(O1_count,2)=mean(sortedM2(sortedM1([9 10 13 14  17 18 21 22 ],2),2)); % go High
BDM_2_Ex_O1(O1_count,3)=mean(sortedM2(sortedM1([ 38 41 42 45 46 49 50 53],2),2)); % stop Low
BDM_2_Ex_O1(O1_count,4)=mean(sortedM2(sortedM1([39 40 43 44 47 48 51 52],2),2)); % go Low
...
% 
    case 2
%  
BDM_1_Ex_O2(O2_count,1)=mean(sortedM1([9 10 13 14  17 18 21 22 ],1)); %stop High ?
BDM_1_Ex_O2(O2_count,2)=mean(sortedM1([  8 11 12 15   16 19 20 23],1)); % go High
BDM_1_Ex_O2(O2_count,3)=mean(sortedM1([  39 40 43 44 47 48 51 52 ]  ,1)); % stop Low
BDM_1_Ex_O2(O2_count,4)=mean(sortedM1([   38 41 42 45 46 49 50 53  ],1)); % go Low
BDM_1_Ex_O2(O2_count,5)=subjects(subjInd); % go Low
BDM_1_Ex_O2(O2_count,6)=order; % go Low


BDM_2_Ex_O2(O2_count,1)=mean(sortedM2(sortedM1([9 10 13 14  17 18 21 22  ],2),2)); %stop High ?
BDM_2_Ex_O2(O2_count,2)=mean(sortedM2(sortedM1([8 11 12 15   16 19 20 23],2),2)); % go High
BDM_2_Ex_O2(O2_count,3)=mean(sortedM2(sortedM1([39 40 43 44 47 48 51 52 ],2),2)); % stop Low
BDM_2_Ex_O2(O2_count,4)=mean(sortedM2(sortedM1([38 41 42 45 46 49 50 53],2),2)); % go Low
